...
class Trainer():
    ...
    def generate_sample(self, n_sample, initial_temperature, time, past_infos, number_ts):
        with torch.no_grad():
            ...
            # Divising temperature and time
            past_temperature = past_infos[0].repeat(number_ts, 1, 1)
            past_time = past_infos[1].repeat(number_ts, 1, 1)

            # Converting to tensor
            time = torch.tensor(time, dtype=torch.float32)
            generated_sample = torch.zeros(
                (number_ts, len_sequence, len(initial_temperature)))

            # Generating the noise
            noise = torch.empty((number_ts, len_sequence, len(
                initial_temperature))).normal_(mean=0, std=1)
            ...
            for i in range(len_sequence):
                ...
                past_temperature = torch.cat(
                    [past_temperature[:, 1:, :], temperature.unsqueeze(1)], axis=1)
                past_time = torch.cat(
                    [past_time[:, 1:, :], time[i].repeat(number_ts, 1).unsqueeze(1)], axis=1)
